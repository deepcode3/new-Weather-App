...
import "../Containers/styles/Fav.css";
...
const Favorite = (props) => {
  const { favcount, setFavcount } = useContext(WeatherContext);
  //const { favcount, setFavcount } = props;
  ...
  let Fav = Data.slice(1).map((e, key) => {
    let Icon = null;
    let smallIcon = null;
    ...
    return (
      <div className="favBox" key={key}>
        <span className="placename">{`${e.name},${e.sys.country}`}</span>
        <img src={smallIcon} alt="weatherIcon" className="wicon" />
        <div className="temp">
          <span className="val">{`${parseInt(e.main.temp)}`}</span>
          <span className="unit">o C</span>
        </div>
        <span className="placename">{`${e.weather[0].description}`}</span>
